...
import { X, Calendar, User, Clock, AlertTriangle, Link2, Check, ChevronDown } from 'lucide-react';
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4" id="modal-container">
        ...
        <motion.div
          ...
          className="relative bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col z-10 max-h-[90vh]"
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-6 flex-1 overflow-y-auto text-sm text-slate-700 dark:text-slate-300 pointer-events-auto" id="modal-form-element">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              {/* Custom Parent Task Dropdown */}
              <div id="grp-parent-task" className="md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider uppercase mb-1.5 flex items-center gap-1">
                  <Link2 className="w-3.5 h-3.5" /> Parent Task (Optional)
                </label>
                <div className="relative" ref={parentDropdownRef}>
                  <button
                    type="button"
                    onClick={() => canEditAnythingElse && setIsParentDropdownOpen(!isParentDropdownOpen)}
                    disabled={!canEditAnythingElse}
                    className={`w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/30 border ${isParentDropdownOpen ? 'border-indigo-500 ring-2 ring-indigo-100 dark:ring-indigo-950' : 'border-slate-200 dark:border-slate-800/80'} rounded-xl flex items-center justify-between text-slate-800 dark:text-slate-100 font-sans transition-all disabled:opacity-60 disabled:cursor-not-allowed`}
                    id="btn-parent-task-dropdown"
                  >
                    <span className="truncate pr-4 text-sm">
                      {parentId === '' ? (
                        <span className="text-slate-500 dark:text-slate-400">None (Top-level Task)</span>
                      ) : (
                        availableDependencies.find(t => t.id === parentId)?.name || 'Unknown Task'
                      )}
                    </span>
                    <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isParentDropdownOpen ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  <AnimatePresence>
                    {isParentDropdownOpen && (
                      ...
                      >
                        <div className="max-h-60 overflow-y-auto scrollbar-thin py-1" id="parent-task-options-list">
                          <button
                            type="button"
                            onClick={() => {
                              setParentId('');
                            ...
                            className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between group ${parentId === '' ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}
                          >
                            <span>None (Top-level Task)</span>
                            {parentId === '' && <Check className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />}
                          </button>
                          
                          {availableDependencies.length > 0 && <div className="h-px bg-slate-100 dark:bg-slate-800 my-1 mx-2" />}
                          
                          {availableDependencies.map(task => {
                            const isSelected = parentId === task.id;
                            return (
                              <button
                                key={task.id}
                                type="button"
                                ...
                                className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between group ${isSelected ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}
                              >
                                <div className="flex-1 min-w-0 pr-3">
                                  <span className="truncate block font-medium">{task.name}</span>
                                  <span className="block text-[10px] text-slate-400 dark:text-slate-500 font-mono mt-0.5 truncate">
